...
def append_feedback(content: str) -> str:
    ...
    feedback_section = f"\n\n### Feedback\nWe value your experience using this tool. Please share your feedback and suggestions [here]({FEEDBACK_FORM_URL})."
    return content + feedback_section
...
Ensure each technology is distinct, actionable, and leverages Schaeffler's €25 billion scale and global innovation network."""
    ...
    # Encourage well‑structured Markdown formatting
    prompt_text += "\n\nFormat your response using Markdown. Use bold headings for each section, bullet points for lists, and tables when presenting comparative data (e.g., TAM/SAM/SOM values). This will ensure the output is clear and professional for users."
...
Use Markdown formatting: start each section with a heading (## or ###), use a table for the assessment dimensions as defined below, and include bullet lists where appropriate. This will help present your answer professionally.
...
Format your response using Markdown. Introduce the radar classification and horizon mapping with subheadings (###) and use bullet points for action plans. Avoid long paragraphs.
...
"""
    # Generate the implementation roadmap and append the feedback invitation
    result = call_llm(p, max_tokens=1800)
    return append_feedback(result)
...
- **Risk Mitigation**: [partnership risk considerations]"""
    # Generate the partner analysis and append the feedback invitation
    result = call_llm(p, max_tokens=800)
    return append_feedback(result)
